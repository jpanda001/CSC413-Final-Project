...
def singleLayerHead(input_features, num_anchors, num_classes, grid_depth):
    ...
    convOut = L.convolution2D(input_features, input_features.shape[-1]*2, \
            3, (1,1), activation=True, biases=True, batchNormalization=True \
...
def boxDecoder(YOLO_heads, input_features_size, anchor_layers, num_classes, scale=1.):
    ...
    grid_size = YOLO_heads.shape[1:4]
    ...
    grid = tf.expand_dims(tf.stack(tf.meshgrid(tf.range(grid_size[0]), tf.range(grid_size[1]), \
            tf.range(grid_size[2])), axis=-1), axis=3)

    grid = tf.tile(tf.expand_dims(tf.transpose(grid, perm=[1,0,2,3,4]), axis=0), \
            [tf.shape(YOLO_heads)[0], 1, 1, 1,  len(anchor_layers), 1])

    grid = tf.cast(grid, tf.float32)

    # Applying formula to derive bounding boxes
    strides = np.array(input_features_size) / np.array(list(grid_size))

    pred_centers = ((tf.sigmoid(centers_raw) * scale) - 0.5 * (scale - 1) + grid) * \
                strides

    pred_sizes = tf.exp(sizes_raw) * anchor_layers
    ...
    return raw_predictions, tf.concat([pred_combined, conf_pred, prob_pred], axis=-1)
